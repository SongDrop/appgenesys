...
exports.DiffApplicationSystem = void 0;
const vscode = require("vscode");
const fs = require("fs");
const path = require("path");
class DiffApplicationSystem {
    /**
    ...
    applyUnifiedDiff(originalContent, diffText) {
        ...
        try {
            ...
            for (let i = 0; i < lines.length; i++) {
                ...
                const deletionsProcessed = result.appliedLines.filter(l => l > hunkStartOriginal && l <= hunkStartOriginal + hunkLinesOriginal).length;
    ...
    */
    async applyPatchFromFile(patchPath) {
        return new Promise((resolve) => {
            const { exec } = require('child_process');
            exec(`git apply --3way "${patchPath}"`, (error, stdout, stderr) => {
                if (error) {
                    resolve({
                        success: false,
                        output: stdout,
    ...
    */
    extractAndFixDiffFromAIResponse(aiResponse, originalFilePath) {
     ...
     */
    generateUnifiedDiff(original, modified, fileName = 'file') {
     ...
     */
    extractDiffFromAIResponse(aiResponse) {
        const diffPatterns = [
            /```(?:diff)?\s*\n([\s\S]*?)\n```/, // Markdown diff block
            /@@ -[\d,]+ \+[\d,]+ @@[\s\S]*?(?=\n\n|\n$|$)/, // Raw diff
            /--- a\/.*?\n\+\+\+ b\/.*?\n@@ -[\d,]+ \+[\d,]+ @@[\s\S]*?(?=\n\n|```|$)/ // Full unified diff
     ...
     */
    saveAISuggestionToHelpFile(filePath, suggestion) {
        try {
            const fs = require('fs');
            const path = require('path');
            ...
            fs.writeFileSync(helpFilePath, JSON.stringify({ comments }, null, 2), 'utf-8');
            return true;
        }
        catch (error) {
     ...
     */
    loadCommentsFromHelpFile(filePath) {
        try {
            const fs = require('fs');
            const path = require('path');
            const dir = path.dirname(filePath);
     ...
     */
    saveAsGitPatch(diff, originalFilePath, patchName, model = 'unknown', query = '', explanation = '') {
        try {
...
`;
     ...
     */
    makeGitCompatible(diffText, fileName) {
        const lines = diffText.split('\n');
        // Check if already has proper headers
        const hasProperHeaders = lines.some(line => line.startsWith('--- a/') && !line.includes('filename.ext'));
        if (hasProperHeaders) {
            return diffText;
        }
     ...
     */
    async checkGitPatch(patchPath) {
        return new Promise((resolve) => {
            const { exec } = require('child_process');
            exec(`git apply --check "${patchPath}"`, (error) => {
                if (error) {
                    resolve({
                        canApply: false,
...
exports.DiffApplicationSystem = DiffApplicationSystem;
// Key Features:
...
//# sourceMappingURL=diff_application.js.map