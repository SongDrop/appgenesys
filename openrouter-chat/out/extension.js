...
const diff_application_1 = require("./diff_application");
// Global instances
let aiPanel = null;
let diffSystem = null;
function activate(context) {
    ...
    diffSystem = new diff_application_1.DiffApplicationSystem();
    // Create and register the webview provider
    aiPanel = new webview_1.openrouterPanel(context, diffSystem);
    ...
    const commands = [
        ...
        vscode.commands.registerCommand('openrouter.applyGitPatch', async (patchPath) => {
            if (!patchPath) {
    ...
    context.subscriptions.push(vscode.window.registerWebviewViewProvider('openrouterChatView', aiPanel));
...
// ============================================================================
async function applySolutionFromComments() {
    const editor = vscode.window.activeTextEditor;
    if (!editor) {
    ...
    if (result.success) {
        ...
        if (applied) {
            vscode.window.showInformationMessage(`✅ Solution applied! ${result.totalChanges} changes made.`);
            // Show summary
            showDiffSummary(result);
        }
    }
    else {
        vscode.window.showErrorMessage(`❌ Failed to apply solution:\n${result.errors.join('\n')}`);
    }
}
async function extractAndApplyDiffFromAI() {
    const editor = vscode.window.activeTextEditor;
    if (!editor) {
        vscode.window.showErrorMessage('No active editor found');
        return;
...
async function validateDiffInEditor() {
    const editor = vscode.window.activeTextEditor;
    if (!editor) {
        vscode.window.showErrorMessage('No active editor found');
        return;
    ...
    if (validation.isValid) {
        vscode.window.showInformationMessage(`✅ Diff is valid!${validation.warnings.length > 0 ? '\nWarnings: ' + validation.warnings.join(', ') : ''}`);
    }
    else {
        vscode.window.showErrorMessage(`❌ Invalid diff:\n${validation.errors.join('\n')}`);
    }
}
async function createDiffBetweenVersions() {
    const editor = vscode.window.activeTextEditor;
    if (!editor) {
        vscode.window.showErrorMessage('No active editor found');
        return;
...
async function generateGitDiff() {
    const editor = vscode.window.activeTextEditor;
    if (!editor) {
        vscode.window.showErrorMessage('No active editor found');
        return;
...
function showDiffSummary(result) {
